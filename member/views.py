...
def member_list(request):
    ...
    if request.method == 'POST':
        ...
        command = ["sh", "../indy/start_docker/sh_generate_did.sh",
                   containerId, wallet_name, wallet_key, student_id]  # did발급 명령어
        try:
            # 명령어 인자로 하여 Popen 실행
...
def regenerate_did(request):
    if request.method == 'POST':
        ...
        if checkDB(api_key):
            ...
            command = ["sh", "../indy/start_docker/sh_regenerate_did.sh", containerId,
                       did, student_id, email, new_wallet_name, wallet_key]  # did 재발급 명령어
            try:
                # 명령어 인자로 하여 Popen 실행
                process = Popen(command, stdout=PIPE, stderr=PIPE)
                ...
                # server로 복사된 did 열기
                with open('/home/deploy/' + str(student_id) + 'NewWalletID.json') as f:
                    json_data = json.load(f)  # json_data에 json으로 저장
                    # 에러 추가
                    ...
                    student.did_time_hash = hashlib.sha256(
                        cmp1.encode('utf-8')).hexdigest()
                    student.save()
                    os.remove('/home/deploy/' + str(student_id) +
...
def findmyinfo(request):
    if request.method == 'GET':
        ...
        info_dump = str(stdnum) + str(major) + str(name) + \
